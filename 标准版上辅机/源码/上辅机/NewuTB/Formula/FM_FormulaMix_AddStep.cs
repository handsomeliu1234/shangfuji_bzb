...
{
    public partial class FM_FormulaMix_AddStep : Form
    {
        ...
        private FormulaMix MixModel;
        private List<FormulaMix> MixList;

        public FM_FormulaMix_AddStep(FormulaMix model, string _deviceId, string _devicePartId, List<FormulaMix> _list, int _stepOrder1)
        {
            InitializeComponent();

            MixModel = model;
            controlCode = MixModel.ActionControlCode;
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            ...
            MixModel.DevicePartID = DevicePartID;
            MixModel.DevicePartCode = NewuGlobal.DevicePartCodeByID(DevicePartID);

            MixModel.StepOrder = stepOrder;
            MixModel.StepCode = GetStepSumBit(out string stepStr);
            MixModel.StepDesc = stepStr;
            MixModel.ActionControlCode = cmb_ActionControlCode.SelectedValue.ToString();

            List<SYS_ActionControl> actionControlList = actionControlRepository.GetList(" ActionControlCode='" + MixModel.ActionControlCode + "'");
            ...
            if (FunClass.IsEmptyOrNumber(cmb_StepPress.Text) != 0)
            {
            ...
            {
                MessageBox.Show(NewuGlobal.GetRes("000251") + NewuGlobal.GetRes("000253"), NewuGlobal.GetRes("000170"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            MixModel.StepTime = FunClass.VVal(txt_StepTime.Text);
            MixModel.StepTemp = FunClass.VDecimal(txt_StepTemp.Text);
            MixModel.StepPower = FunClass.VVal(txt_StepPower.Text);
            MixModel.StepEnergy = FunClass.VDecimal(txt_StepEnergy.Text);
            MixModel.StepPress = FunClass.VDecimal(cmb_StepPress.Text);
            MixModel.StepSpeed = FunClass.VDecimal(cmb_StepSpeed.Text);
            MixModel.KeepTime = FunClass.VVal(txt_KeepTime.Text);
        ...
        private void FM_FormulaMix_AddStep_Load(object sender, EventArgs e)
        {
        ...
        private void InitPage()
        {
            ...
            if (MixModel.ActionControlCode != null)
                cmb_ActionControlCode.SelectedValue = MixModel.ActionControlCode;
            txt_StepTime.Text = MixModel.StepTime.ToString();
            txt_StepTemp.Text = MixModel.StepTemp.ToString();
            txt_StepPower.Text = MixModel.StepPower.ToString();
            txt_StepEnergy.Text = MixModel.StepEnergy.ToString();
            cmb_StepPress.Text = MixModel.StepPress.ToString();
            cmb_StepSpeed.Text = MixModel.StepSpeed.ToString();
            txt_KeepTime.Text = MixModel.KeepTime.ToString();
            ...
            actionStepRepository.FindCheckListItemByMixStepCode(MixModel.StepCode, boxs, MixModel.StepDesc);
        ...
        private void BtnClose_Click(object sender, EventArgs e)
        {
            ...
            {
                MixModel.ActionControlCode = controlCode;
            }
        ...
        private string ComputeDropOrder(int stepOrder)
        {
            int computerOrder = 0;
            int cishu = 0;
            foreach (FormulaMix item in MixList)
            {
